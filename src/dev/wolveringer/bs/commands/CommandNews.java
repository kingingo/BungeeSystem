	...
	public void execute(CommandSender sender, String[] args) {
		...
		if (PermissionManager.getManager().hasPermission(player, PermissionType.MOTD,true)) {
			if(args.length==0){
				...
				player.sendMessage(Language.getText(player, "PREFIX")+"/news remove [LANGUAGE] [B/T] [ID]");
				player.sendMessage(Language.getText(player, "PREFIX")+"/news add [LANGUAGE] [B/T] [NEWS]");
			}else{
				if(args[0].equalsIgnoreCase("list")){
					...
					if(args[2].toLowerCase().startsWith("b")) args[2]="BROADCAST";
					else if(args[2].toLowerCase().startsWith("t")) args[2]="TITLE";
					else{
